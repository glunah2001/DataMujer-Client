...
public class NewPaymentController extends BaseSubSceneController {
    ...
    private boolean validateData(){
        if(txtDescription.getText().trim().isEmpty()){
            UIUXFeedbackUtils.showErrorSnackbar("Por favor, complete toda la información del pago");
            return false;
        }
        ...
        if(amount.compareTo(BigDecimal.ZERO) <= 0) {
            UIUXFeedbackUtils.showErrorSnackbar("El monto total debe ser mayor que 0");
            return false;
        }
    ...
    public BigDecimal getTotalAmountValue() {
        try {
            ...
            UIUXFeedbackUtils.showErrorSnackbar("Ocurrió un error al convertir el valor monetario.");
            return null;
        }
    }

...
public class PaymentUpdateController extends BaseSubSceneController{
    ...
    private void update(ActionEvent event) {
        ...
        mainController.executeCall(
                ...
                (PaymentDTO dto) -> {
                    mainController.showSuccessSnackBar("Éxito en el reporte de actualización de pago #"+dto.id()
                    +" a estado: PAGADO");
                    mainController.withDelay(4, () -> {
                        mainController.hideLoading();
                        DataUtilities.clearPaymentDTO();
                        mainController.loadPayments(null);
                    });
                },
                (ApiError error) -> {
                    if(error.status() == 404){
    ...
    private boolean validateData(){
        if(dtpPayDate.getValue() == null){
            mainController.showErrorSnackBar("Por favor, seleccione una fecha válida.");
            return false;
        }
